                    ...
                    if (
                      ...
                      if (self.opts.compileDebug) {
                        includeSrc =
                          ...
                          "\n" +
                          "      , __filename = " +
                          JSON.stringify(includeObj.filename) +
                          ";" +
                          "\n" +
                          "      try {" +
                          "\n" +
...
function FormFacade(data) {
  ...
  this.computeField = function (tmpl) {
    ...
    params.num = function (val) {
      if (val) {
        if (isNaN(val) == false) return Number(val);
      }
      return 0;
    };
    params.ifs = function (...args) {
      var lst;
      if (args.length % 2 == 1) lst = args.pop();
  ...
  this.init = function (savedId) {
    ...
    if (savedId && !this.data.request.query.flush) {
      ...
      savedprm = new Promise(function (resolve, reject) {
        ...
        xhr.open(
          "GET",
          baseurl + "/draft/" + publishId + "/read/" + savedId,
          true
        );
        xhr.responseType = "json";
        xhr.onload = function () {
          if (xhr.status === 200) {
            if (xhr.response && xhr.response.entry) resolve(xhr.response);
          }
        };
        xhr.send();
      });
    ...
    return savedprm.then(function (drft) {
      if (drft && drft.entry) curr.draft = drft;
      else curr.draft = curr.prefill();
    });
  };
  ...
  this.load = function (divId) {
    var curr = this;
    if (!window.Promise)
      return this.polyfill(function () {
        curr.load(divId);
      });
    if (this.data.request.params.target == "classic") {
      if (window.wp) this.data.request.params.target = "wordpress";
    }
    this.init().then(function () {
      curr.divId = divId;
      curr.render();
      var callback = curr.data.request.query.callback;
      if (callback && window[callback]) window[callback](curr);
      curr.scrapeSection();
    });
  };
  ...
  this.popup = function (qry) {
    ...
    if (!window.jQuery && qry.onexit) {
      ...
      return this.loadScript(jqsrc, function () {
        curr.popup(qry);
      });
    }
    this.init().then(function () {
      ...
      if (curr.config.themecss && false) {
        ...
        if (qhead && qbody) {
          var hfont = window
            .getComputedStyle(qhead, null)
            .getPropertyValue("font-family");
          var bfont = window
            .getComputedStyle(qbody, null)
            .getPropertyValue("font-family");
      ...
      if (qry.delay) {
        ...
        var lbtns;
        if (qry.onclick) lbtns = document.querySelectorAll("#" + qry.onclick);
        else lbtns = document.querySelectorAll(qry.selector);
        ...
        else dt = new DialogTrigger(cback, { trigger: "exitIntent" });
      }
      var callback = curr.data.request.query.callback;
  ...
  this.showPopup = function () {
    ...
    var ifr = document.getElementById("ffcontent");
    if (ifr.contentWindow.showModal) ifr.contentWindow.showModal();
    else
      setTimeout(function () {
        curr.showPopup();
      }, 500);
  ...
  this.isPreviewMode = function () {
    if (window.editFacade) return true;
  ...
  this.entry = function (entryId) {
    if (this.draft && this.draft.entry) {
      ...
      else this.draft.entry[entryId.toString()];
    }
  };
    ...
    editor.blocks.forEach(function (block) {
      if (!block.data) {
        ...
        if (block.data.level <= 4) {
          ...
          htm =
            '<small class="ff-help form-text text-muted">' +
            block.data.text +
            "</small>";
        }
        htmls.push(htm);
  ...
  this.insideIframe = function () {
    return this.data.request.params.target == "popup" ||
      this.data.request.params.target == "iframe"
      ? true
      : false;
  };
  ...
  this.resizeIframe = function () {
    if (this.data.request.params.target == "iframe") {
      ...
      var hgh = obj.contentWindow.document.getElementById("ffContent")
        .scrollHeight;
  ...
  this.getContentElement = function () {
    ...
    } else elm = document.querySelector(this.divId);
    return elm;
  };
  ...
  this.render = function () {
    ...
    if (frm) {
      frm.addEventListener("change", function (evt) {
        ...
        if (userId)
          httpurl =
            baseurl + "/draft/" + userId + "/form/" + publishId + "/save";
  ...
  this.submit = function (frm, secid) {
    ...
    while ((next = entries.next()) && next.done === false) {
      ...
      if (val) val.push(entry[1]);
      else pairs[entry[0]] = [entry[1]];
    }
    ...
    this.sendData(pairs)
      .then(function (rs) {
        ...
        if (rs && rs.code == 200) {
          ...
          frm.action =
            "https://docs.google.com/forms/d/e/" + publishId + "/viewform";
          frm.method = "GET";
          frm.submit();
        } else {
          formFacade.render();
  ...
  this.submitData = function (nmval) {
    ...
    for (var itemId in items) {
      ...
      if (val && item.entry) pairs["entry." + item.entry] = val;
    }
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (!trgurl && params.userId && params.publishId) {
        if (savedId)
          ...
          url =
            url + "/" + params.userId + "/form/" + params.publishId + "/draft";
      }
      ...
      for (var nm in pairs) {
        ...
        } else if (val) params += "&" + nm + "=" + encodeURIComponent(val);
      }
      var http = new XMLHttpRequest();
      ...
      http.onload = function () {
        ...
        if (http.status == 200) resolve(jso);
        if (http.status == 201) resolve(jso);
        else if (http.status >= 400) reject(jso);
      };
      http.send(params);
    });
  };
  ...
  this.validate = function (frm, secid) {
    ...
    secdiv.querySelectorAll(wids.join(", ")).forEach(function (wid, w) {
      ...
      if (
        valid &&
        wid.className &&
        wid.className.indexOf("ff-grid-radio") >= 0
      ) {
      ...
      if (!valid && invalids == 0) {
        wid.focus();
        invalids = invalids + 1;
        if (wid.reportValidity) wid.reportValidity();
      }
    });
    return invalids;
  };
  ...
  this.gotoSection = function (frm, secid, deftrg) {
    ...
    if (deftrg == "back") {
      ...
      var items = this.data.scraped ? this.data.scraped.items : {};
      doc
        .querySelectorAll("#ff-sec-" + secid + " .ff-nav-dyn")
        .forEach(function (wid, w) {
          if (wid.id) {
      ...
      if (trg == -1) trg = secid;
      else if (trg == -2) trg = this.getNextSectionId(secid);
      else if (trg == -3) trg = "ending";
    }
  ...
  this.scrapeSection = function (pghistory) {
    ...
    this.sendData(pairs, "/nextSection/" + publishId)
      .then(function (rs) {
        if (rs && rs.blobs && curr.data.form) {
          var exblobs = curr.data.form.blobs;
          curr.data.form.blobs = Object.assign(
            exblobs ? exblobs : {},
            rs.blobs
          );
          //curr.render();
  ...
  this.getDocument = function () {
    var doc = document;
    if (
      this.data.request.params.target == "popup" ||
      this.data.request.params.target == "iframe"
    ) {
      var ifr = document.getElementById("ffcontent");
  ...
  this.uploadFile = function (fld, entry, widg) {
    ...
    return new Promise(function (resolve, reject) {
      var publishId = curr.data.request.params.publishId;
      var savedId =
        curr.draft && curr.draft.savedId ? curr.draft.savedId : "none";
      var url =
        "https://formfacade.com/upload/" +
        publishId +
        "/" +
        savedId +
        "/" +
...
window.formFacade = new FormFacade({
  form: {
    at: 1587354631716,
    description: "",
    id: "17IGfsBL5U97kPENT9xTXvosZvNtfjAkFc5IzLb2SVZ0",
    items: {
      "366990283": { index: 0, required: true, title: "Name", type: "TEXT" },
      "773651004": { index: 3, required: true, title: "City", type: "TEXT" },
      "1058152648": {
        index: 5,
        required: true,
        title: "Message",
      ...
      "1775148053": { index: 1, required: true, title: "Email", type: "TEXT" },
    },
    title: "SMB Surgical Inquiry",
    url:
      "https://docs.google.com/macros/s/AKfycbzK8bQpDCcU68BPtlaezQ5Smws-NE-N_jA6LQyFIxEz_Mywqq_-/exec",
  },
  facade: {},
  scraped: {
    at: 1587354870836,
    emailAddress: 2,
    form: "SMB Surgical Inquiry",
    items: {
      "366990283": {
        entry: 433138137,
        index: 0,
  ...
  util: {
    ...
    codeTemplate: {
      default: [
        ...
        '<script async defer src="https://formfacade.com/forms/d/e/{id}/viewform.js?div=ff-compose"></script>',
      ],
      bootstrap: [
        '<div class="container" id="ff-compose"></div>',
        '<script async defer src="https://formfacade.com/forms/d/e/{id}/bootstrap.js?div=ff-compose"></script>',
      ],
      wordpress: ["[formfacade id={id}]"],
      squarespace: [
        '<div class="container" id="ff-compose"></div>',
        '<script async defer src="https://formfacade.com/forms/d/e/{id}/squarespace.js?div=ff-compose"></script>',
      ],
      popup: [
        '<script async defer src="https://formfacade.com/forms/d/e/{id}/popup.js?delay=1sec"></script>',
      ],
      poponclick: [
        '<button id="ff-launch-popup" style="/* change button style */">Your button</button>',
        '<script async defer src="https://formfacade.com/forms/d/e/{id}/popup.js?onclick=ff-launch-popup"></script>',
      ],
    },
    Departments: {
      marketing: "Marketing",
    ...
    Industries: {
      ...
      energy: "Energy, Chemicals, & Utilities",
    ...
    Templates: {
      "11s_aI346DuxjDlX1r6jM-6W7KPQItVod3splfK-rc_k": {
        name: "Registration form",
        publishId: "1FAIpQLSdE7U4WU5mgXWUdwg_btumi__qXsX8EfCCmpvRB2yB0-Ut72g",
        description:
          "Signup attendees for your event with the registration form",
        dashboard:
          "2PACX-1vQENZZwJdaEXMVFwUxJ8pkkY-artvLzkaYjTNN0oJnxPlbU_ltSEmtXcJ5kCYmpCaOSX-c-BieFRqx2",
      },
      "1FSdOEoOrFd3COUi6LILlWQx3lA6BvjqDBv1jewzMk30": {
        name: "Order form",
        publishId: "1FAIpQLSff02m5k4-kSjU-OgmM5qNxxo2lxlKZgRq30ACGdOLikuVGNA",
        description: "Collect orders for your products with this order form",
        dashboard:
          "2PACX-1vQAkdyoRXIB2PqI8n-MqwAUQpfJUbm6YTNQQfqI7ges0L7QTq8QCNfeXUXjTp9CLJBMZlN_3SNuqgXK",
      },
      "1lx-uW_2pO-LtLwankk-L1rjMTOM4GiA68rCJPxTkTgs": {
        name: "Leave request form",
        publishId: "1FAIpQLSdmHPbOU07-jGh_AEbQc5iydvRZm7otJB2T8ObWjvJdNcYs-A",
        description:
          "Manage employee leave requests with this leave request form",
        dashboard:
          "2PACX-1vQTSL94wiO2c8yIywntwalZ9kWmYr4yHHxPnCNSvaq57CbQR_iBy2KJaUNndsoKJDOjEG8KG5C3qdbe",
      },
      "1ihoDhrDlT_rad3POOluU_RvvHzck3Jy54l4gIioma-I": {
        name: "Feedback form",
        publishId: "1FAIpQLSe5YRO8LhjEl8YQqNDKcRkRFFwin-cbty4ZliZUVehegZWBaw",
        description:
          "Get feedback about the ordering experience from your users with this feedback form",
        dashboard:
          "2PACX-1vQENZZwJdaEXMVFwUxJ8pkkY-artvLzkaYjTNN0oJnxPlbU_ltSEmtXcJ5kCYmpCaOSX-c-BieFRqx2",
      },
      "1okN7LMlXD2lwjxTkPbEzqDcMymEncjVDVAi8XsbxsU8": {
        name: "Contact form",
        publishId: "1FAIpQLSeXTbNm2FaDWVeJ_86fTue0MaQScTpRwJE0-CQGomgjOgwQRw",
        description: "A simple contact form template to capture leads",
        dashboard:
          "2PACX-1vQ9tWvOxeMQ9ryZLPtaBllYW-16cTujfiDTE6OXtyjqUPnMaNc4_RsNZQaOJ2Cam4hT7PoywYNujZnw",
      },
      "1rvKwg_PUVGEh9MSM2OYubtSWqIRZWd11vCz7WKGSXhQ": {
        name: "Blank form",
        publishId: "1FAIpQLSchUxyAWL9PsNvF6yL1N4rBl7N-utpNBYxcyj12MONIwnDQsA",
        description:
          "A blank form template with the recommended settings to use Formfacade",
      },
    },
    faqs: {
      "1FAIpQLSei5hj5tKb5RhKwnUYaYOoO3Ua_5Q-ja0bnF3evk9qSptr44Q":
        "Can I customize the thank you message shown after form submission?",
      "1FAIpQLSdE7NT6JzjGs95E5xSX1Hm5cOV5l5_KALUC0W-V4fyM9nNE2Q":
        "Can I change the language for the button text in Formfacade?",
      "1FAIpQLSe5CMdIn8rh34-7MGOTAxXGYw_dO-unFX1l517KcgQLis83Ew":
        "Is there a way to change the button color or font styles in the form?",
      "1FAIpQLSfGvg22V7Lzyw_5AEbKBSpklS_TMw6tKxcQiDqlC9KvfBVTgQ":
        "Does Formfacade support pre-filled survey links like native Google Forms?",
      "1FAIpQLSf-QvZdKITPWh2bFKLurcc2UlMEzZR1cAgClCltyQKpY6uz_w":
        'Getting "Form requires sign-in" error message',
      "1FAIpQLSdRM0Mf6lRhJnwsIB5iS5XwbY02LRlhwZ7WygW67uxHuMOQsA":
        "Redirects to Google Form on submit, shows an error message",
      "1FAIpQLSdph4zqqMX35jab06ZDBsT3zathdcatHn_bqgo4PbJ_wD3-Og":
        "How to organize and manage form responses?",
      "1FAIpQLSfFWtpI3dY6ootFcM4w3KNjxfrFa98D6fPhzIdn4ncR4gW5NQ":
        "How to change the placeholder text in Google Forms?",
      "1FAIpQLScsQxfv3qbfi-qzk7Xty7IIq8ifoGih3d-LAN5EBIPJXgUaDA":
        "Is it possible to hide fields in Google Forms?",
      "1FAIpQLSdsOMlrcCn5Q1T_flqI6XAjBKd9jY-da9awzBJLyvoKksdjDA":
        "Can I add a file upload option in my forms?",
      "1FAIpQLSfyhpvJmKw3qsS_JcsEmZqMWBbbWHCSkS2e4dTABPtJc5b4Cg":
        "How can I transfer ownership of the form?",
      "1FAIpQLSeooiyPfimYtFV-LlN3uiXUNu3QbPzVxCa33BCSGrPR2AqwcA":
    ...
    paid: [
      "106562640896077409923",
      "103035429268472736353",
      "117025644207735429235",
      "104951976458623252931",
      "103139092767592738742",
      "117404790123818520387",
      "108539250703100582879",
      "109871848110379814860",
      "116726145860011181529",
      "102174788568626953401",
      "112390932842225524671",
      "115838340203454328973",
      "103060768793271002536",
      "117607568578654866325",
      "106369147526416463296",
      "102783377159500204753",
      "116374076704682468134",
      "110825628153483398189",
      "113415303342754916783",
      "117780956667207983482",
      "104416910406745232840",
      "105406797656242780603",
      "118072050622885292690",
      "105242287939464144485",
      "100346974933055186208",
      "108501825478818400409",
      "107870735454189233937",
    ],
    warned: [
      "117286938286319180732",
      "105925741395796620521",
      "116089346103608674243",
    ],
    blocked: [
      "108850108991157602986",
      "102689789065446473820",
      "108671534328667933178",
      "117402393743386075821",
      "117522479448611613376",
      "104809234324266850030",
      "102603499279084578584",
      "107810942555344101481",
      "112334250667158698219",
      "107866052644851006308",
      "118072088318591257000",
      "111045421501054835580",
      "104141395229590358558",
      "113239234620458768547",
      "103228058283134885615",
      "105931539075667801352",
      "100179101893592656163",
      "116506754827270429348",
      "101022414382305081205",
      "106800933712001214281",
      "106339925407449534697",
      "106961864884757189460",
      "116564932480417711472",
      "114954407903021190865",
      "105243818915915840501",
      "117879770688136236363",
      "116912257312448702030",
      "112322559097344885129",
    ],
    gsuiteurl:
      "https://gsuite.google.com/marketplace/app/formfacade/743872305260",
  },
  request: {
...
formFacade.config = {
  ...
  themecss:
    "font=%22Work%20Sans%22%2CHelvetica%2CArial%2Csans-serif&heading=%22Poppins%22%2C%20sans-serif&primary=%235d33fb&primaryActive=%23492bbb&secondary=%23b161fc",
};
